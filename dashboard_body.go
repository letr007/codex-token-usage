...
<main>
  ...
  <section class="section" style="margin-top:8px"><h2><span>429 自动禁用状态</span><span class="mini">Codex reset_at 自动恢复</span></h2><div class="section-body"><div class="autoban-toolbar"><span id="autoban-scope" class="autoban-scope">显示 0 / 0 个自动禁用账号</span><select id="autoban-page-size" aria-label="自动禁用每页数量"><option value="10">10 / 页</option><option value="25">25 / 页</option><option value="50">50 / 页</option></select><button id="autoban-prev" class="ghost" aria-label="上一页自动禁用账号">上一页</button><span id="autoban-page-label" class="page-label">1 / 1</span><button id="autoban-next" class="ghost" aria-label="下一页自动禁用账号">下一页</button></div><div class="scroll autoban-table-wrap"><table><thead><tr><th>账号</th><th>AuthIndex</th><th>窗口</th><th>原因</th><th>封禁时间</th><th>解禁时间</th><th>剩余</th><th>5h</th><th>7d</th></tr></thead><tbody id="autobans"></tbody></table></div></div></section>
